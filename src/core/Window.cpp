...
void Window::ShowTooltip(int x, int y, const std::wstring& text) {
    if (!m_tooltip) {
        return;
    }
    ...
    SendMessageW(m_tooltip, TTM_UPDATETIPTEXTW, 0, (LPARAM)&ti);
    
    SendMessageW(m_tooltip, TTM_TRACKPOSITION, 0, MAKELPARAM(pt.x, pt.y + 20));
    SendMessageW(m_tooltip, TTM_TRACKACTIVATE, TRUE, (LPARAM)&ti);
}
...
void Window::HideTooltip() {
    ...
    ti.hwnd = m_hwnd;
    ti.uId = 1;
    SendMessageW(m_tooltip, TTM_TRACKACTIVATE, FALSE, (LPARAM)&ti);
}
...
LRESULT Window::WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    int delta = GET_WHEEL_DELTA_WPARAM(wParam);
                ...
                // WM_MOUSEWHEEL lParam contains screen coordinates, convert to client
                ...
                OnMouseWheel(pt.x, pt.y, delta, horizontal);
}
